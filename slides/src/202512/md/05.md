...
UniformGrid1.WholeExtent=[0, 127, 0, 127, 0, 0]
...
## Instrument a mini Heat Diffusion example with Catalyst. Act II

- Revisit our [Heat Diffusion] example
- Inspect CatalystAdaptor.h

[Heat Diffusion]: https://github.com/jfavre/InSitu-Vis-Tutorial/tree/main/Examples/HeatDiffusion/C++
...
srun -u -n 4 --reservation=insitu -t 2 ./buildCatalyst/bin/heat_diffusion --mesh=uniform --res=64 catalyst_state.py
```
...
## Connection LIVE to the Heat Diffusion simulation

- Recall previous page on definining a catalyst script
- that script can run in batch mode, generating images and extracts at the given frequencies.
...
```
<br>
- start a ParaView client and Catalyst=>connect()<br>
- ssh -R 22222:localhost:22222 daint-ln004<br>
- cd InSitu-Vis-Tutorial/Examples/HeatDiffusion/C++<br>
- sbatch --reservation=insitu run_catalyst.sh<br>
- Live demo
...
- Main problems: it is an N-to-N mapping:<br>
<v-click>&nbsp;&nbsp;&nbsp; - visualization algorithms probably don't scale with the same granularity of simulations</v-click><br>
<v-click>&nbsp;&nbsp;&nbsp; - The simulation waits while visualization steps take place</v-click><br>

<v-click>&nbsp;&nbsp;&nbsp; - ParaView Catalyst does not support (Conduit) data on the device.</v-click><br>
<v-click>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp - But we have a partial solution (see the CUDA-enabled Heat Diffusion example slides)</v-click>
